...
while ($row = sql_fetch_array($result))
{
    ...
    if (!$row['wr_is_comment'])
    {
		...
		//기타 비스켓 전용 테이블 자료 삭제
		sql_query("delete from beesket_sale where bo_table = '{$bo_table}' and wr_id = '{$row['wr_id']}';");
		sql_query("delete from beesket_price where bo_table = '{$bo_table}' and wr_id = '{$row['wr_id']}';");
		sql_query("delete from beesket_language where bo_table = '{$bo_table}' and wr_id = '{$row['wr_id']}';");
